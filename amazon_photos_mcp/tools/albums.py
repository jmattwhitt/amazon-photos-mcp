...
@_tool
async def list_albums(max_results: int = 100) -> dict[str, Any]:
...
@_tool
async def create_album(name: str) -> dict[str, Any]:
    """Create a new album in Amazon Photos."""
    ap = _get_client()
    result = await ap.create_album(name)
...
@_tool
async def add_to_album(album_id: str, node_ids: list[str]) -> dict[str, Any]:
    """Add photos/videos to an existing album."""
    ap = _get_client()
    result = await ap.add_to_album(album_id, node_ids)
...
@_tool
async def remove_from_album(album_id: str, node_ids: list[str]) -> dict[str, Any]:
    """Remove photos/videos from an album (does not delete files)."""
    ap = _get_client()
    result = await ap.remove_from_album(album_id, node_ids)
