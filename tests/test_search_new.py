...
async def test_advanced_search_accepts_valid_date_formats(mock_get_client):
    ...
    res2 = await advanced_search(date_from="20240101")
    assert "error" not in res2

    # Empty string should be skipped (no error)
    mock_ap.query.return_value = [{"id": "node1"}]
    res3 = await advanced_search(date_from="")
