...
def mock_amazon_client() -> None:
    ...
    mock_ap.videos.return_value = []
    mock_ap.query.return_value = mock_ap.db
    mock_ap.aggregations.return_value = []
    mock_ap.get_folders.return_value = [{"id": "f1", "name": "Test"}]
    mock_ap.trashed.return_value = []
    ...
    with patch("amazon_photos_mcp.client._get_client", return_value=mock_ap):
        yield
